...
//         public const int POS_FONT_TYPE_COMPRESSED = 0x01;//压缩ASCII
        ...
        /// <returns>如果函数调用成功，返回一个已打开的端口句柄。如果函数调用失败，返回值为 INVALID_HANDLE_VALUE （-1）。</returns>        [DllImport("Library\\POSDLL.dll", CharSet = CharSet.Ansi)]
        [DllImport("Library\\POSDLL.dll", CharSet = CharSet.Ansi)]
        ...
        /// </summary>
        /// <returns></returns>
        [DllImport("Library\\POSDLL.dll", SetLastError = true)]
