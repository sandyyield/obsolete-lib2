...
namespace ZlPos.Dao
{
    class DbManagerImpl : DbManager
    {
            ...
            try
            {
                ...
                using (var db = SugarDao.Instance)
                {
                    ...
                    else
                    {
                        ...
                        //IEnumerable<DataRow> query1 = dt1.AsEnumerable().Except(dt2.AsEnumerable(), DataRowComparer.Default);
                        //还是用转成list性能比较快 
                        var query1 = dt1.AsEnumerable().Except(dt2.AsEnumerable(), DataRowComparer.Default).ToList();
                        //两个数据源的差集集合
                        if (query1.Any())
                        {
                            DataTable dt3 = query1.CopyToDataTable();
        ...
        /// <typeparam name="T"></typeparam>
        /// <param name="array"></param>
        public void BulkSaveOrUpdateTurbo<T>(List<T> dataArr, string Comparer, string primaryKey = "id") where T : class, new()
        {
            T[] array = dataArr.ToArray();
            try
            {
                if (array == null)
                {
                ...
                using (var db = SugarDao.Instance)
                {
                    ...
                    else
                    {
                        ...
                        //list3 = list1.Except(list2,new Comparer()).ToList();



                        //if (string.IsNullOrEmpty(Comparer))
                        //{
                        //    throw new Exception("数据比较器不能为空");
                        //}
                        //else if ("SPU".Equals(Comparer))
                        //{
                        //    var query = data.Except(table,new SPUComparer()).ToList();
                        //}
                        //else if ("SKU".Equals(Comparer))
                        //{
                        ...
                        if (query1.Any())
                        {
                            ProcessData<T>(primaryKey, db, dt2, query1);
        ...
        private static void ProcessData<T>(string primaryKey, SqlSugar.SqlSugarClient db, DataTable dt2, List<DataRow> query1) where T : class, new()
        {
            DataTable dt3 = query1.CopyToDataTable();
